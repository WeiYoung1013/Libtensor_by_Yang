...
*/
    ...
    void print(int precision =-6 , bool raw= false  ) {
        ...
        Tensor *aux = nullptr;
        aux = this;
        auto calculateOffset = [&](const std::vector<int>& shape, const std::vector<int>& stride, int index) -> int {
            int offset = 0;
            for (int i = shape.size() - 1; i >= 0; --i) {
                int dim_index = index % shape[i];
                index /= shape[i];
                offset += dim_index * stride[i];
            }
            return offset;
        };
        ...
        for (int i = 0; i < aux->size_; ++i) {
            ...
            if(raw){
                // Print number
                int offset = calculateOffset(aux->shape, aux->stride, i);
                buffer << aux->ptr[offset] << ", ";
...
// index Indexing and slicing
    ...
    Tensor<T>* shallow_copy() {
        Tensor* t = new Tensor;
        t->ptr = this->ptr;
        t->original_ptr = this->ptr;
        t->shape = this->shape;
        t->size_ = this->size_;
...
//        auto* t = new Tensor(sd->oshape);
        ...
        for (int i = 0; i < this->ndim; ++i) {
            t->ptr += stride[i] * sd->idxs_range[i][0];
            sum *= sd->oshape[i];
        }
        t->size_ = sum;
        t->shape = sd->oshape;
...
// 2.3 Mutating operations
    ...
    void set_select(const vector<string>& indices, T value){
        ...
        for (int i = 0; i < t->size_; ++i) {
            int offset = calculateOffset(t->shape, t->stride, i);
            t->ptr[offset] = value;
        }
    ...
    static Tensor<T>*  transpose(Tensor* A, const vector<int>& dims) {
        // Build descriptor
        vector<int> new_dims(A->ndim);
        for (int i = 0; i < A->ndim; ++i) {
            new_dims[i] = i;
        }
        swap(new_dims[dims[0]], new_dims[dims[1]]);
        auto *sd = new PermuteDescriptor(new_dims);
        sd->build(A->shape);

        auto new_t = A->shallow_copy();
        int ndim = A->ndim;
        std::vector<int> newShape(ndim);
        std::vector<int> newStride(ndim);

        for (int i = 0; i < ndim; ++i) {
            newShape[i] = A->shape[new_dims[i]];
        }

        for (int i = 0; i < ndim; ++i) {
            newStride[i] = A->stride[new_dims[i]];
        }
        new_t->shape = newShape;
        new_t->stride = newStride;
        delete sd;
        return new_t;
...
//        Tensor::select(A, new_t, sd);
        ...
        for (int i = 0; i < ndim; ++i) {
            newShape[i] = A->shape[dims[i]];
        }

        for (int i = 0; i < ndim; ++i) {
            newStride[i] = A->stride[dims[i]];
        }
        new_t->shape = newShape;
        new_t->stride = newStride;
        delete sd;
